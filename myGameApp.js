  ...
  this.draw = function () {
    ...
    for (let i = 0; i < NumRows; i++) {
      for (let j = 0; j < NumCols; j++) {
        if(enemy_ships[i][j].isAlive && Math.abs(enemy_ships[i][j].x-this.x+25)<50&&Math.abs(enemy_ships[i][j].y-this.y-10)<=50){
...
function EnemyFire(x, y){
  ...
  this.final=false;
  this.draw = function () {
    if(Math.abs(friendly_ship.x-this.x)<50&&Math.abs(friendly_ship.y-this.y-10)<=50){
      lives--;
      EnemyFireCount++;
      document.getElementById("heartLI").removeChild(document.getElementById("heartLI").lastChild);
...
function setupGame() {
  var default_user = new User("p", "testuser", "Admin@Admin.com", "01.01.2023", "Admin", "Admin");
  ...
  document.getElementById("SumbitSignUp").addEventListener("click", sumbitSignUp);
...
function logOut() {
  ...
  while(document.getElementById("heartLI").firstChild){
    document.getElementById("heartLI").removeChild(document.getElementById("heartLI").firstChild);
  }
...
function sumbitSignUp() {
  ...
  if (checkSetUp(username, password, email, firstname, lastname, confirmPassword)) {
    users[username] = new User(username, password, email, birthday, firstname, lastname);
    logedInUser = users[username];
...
function checkSetUp(username, password, email, firstname, lastname, confirmPassword) {
...
function resetElements() {
  ...
  friendly_ship = new FriendlySpaceShip(WidthDistanceFactor * Math.random() * canvasWidth, canvasHeight - 80);
  enemy_ships = Array(NumRows).fill().map(() => Array(NumCols));
  for (let i = 0; i < NumRows; i++) {
    for (let j = 0; j < NumCols; j++) {
      enemy_ships[i][j] = new EnemySpaceShip(j * canvasWidth*0.7 / NumCols, i * canvasHeight*0.5 / NumRows);
    }
  }
    ...
    for(let j=0;j<NumCols;j++){
      if(i==y_rand&&j==x_rand){
        if(enemy_ships[i][j].isAlive==true){
          if(EnemyFireCount>0){
            EnemyFireCount--;
            EnemyFireARR.push(new EnemyFire(enemy_ships[i][j].x,enemy_ships[i][j].y));
          }
        }
      }
